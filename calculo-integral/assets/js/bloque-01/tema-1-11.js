...
(() => {
  ...
  function drawLab(){
    ...
    if(NUM==='1.9' || NUM==='1.11'){
      configure(NUM==='1.9'?`<label>Constante C<input id="C" type="range" min="-4" max="4" step=".25" value="1"></label>`:`<label>x₀<input id="labX0" type="range" min="-2" max="2" step=".25" value="1"></label><label>y₀<input id="labY0" type="range" min="-1" max="6" step=".25" value="5"></label>`);
      const run=()=>{let ax=baseAxes(-2.2,2.2,-4.5,8);ax.xmin=-2.2;ax.xmax=2.2;let out=ax.out,selected=NUM==='1.9'?+$('#C').value:(+$('#labY0').value-(+$('#labX0').value)**2);[-3,-1,1,3].forEach(c=>{out+=`<path d="${pathFor(x=>x*x+c,ax)}" class="family"/>`;});out+=`<path d="${pathFor(x=>x*x+selected,ax)}" class="family-selected"/>`;if(NUM==='1.11'){let x0=+$('#labX0').value,y0=+$('#labY0').value;out+=`<circle class="point-mark" cx="${ax.X(x0)}" cy="${ax.Y(y0)}" r="7"/><text x="${ax.X(x0)+10}" y="${ax.Y(y0)-10}">(${x0},${y0})</text>`;}svg.innerHTML=out;setMetrics(fmt(selected),'Constante C','2x','Derivada de x²+C',NUM==='1.11'?'1 condición':'∞','Selección');$('#labFeedback').innerHTML=`Curva destacada: <strong>F(x)=x²${selected>=0?'+':''}${fmt(selected)}</strong>. Todas tienen derivada 2x.`;};
      $$('#labControls input').forEach(e=>e.addEventListener('input',run));run(); return;
    }
    if(NUM==='1.10'){
