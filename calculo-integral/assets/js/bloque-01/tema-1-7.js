...
(() => {
  ...
  function drawLab(){
    ...
    if(NUM==='1.4' || NUM==='1.7'){
      configure(`<label>Función<select id="fn"><option value="x2">x² en [0,1]</option><option value="x">x en [0,1]</option>${NUM==='1.7'?'<option value="sig">Señal 1+0.6·sin(2πt)</option>':''}</select></label><label>Método<select id="labMethod"><option value="left">Izquierda</option><option value="right">Derecha</option><option value="mid" selected>Punto medio</option><option value="trap">Trapecio</option></select></label><label>n<input id="ln" type="range" min="2" max="80" value="8"></label>`);
      const run=()=>{let type=$('#fn').value,method=$('#labMethod').value,n=+$('#ln').value,f=type==='x2'?x=>x*x:type==='x'?x=>x:x=>1+.6*Math.sin(2*Math.PI*x),exact=type==='x2'?1/3:type==='x'?.5:1,ymax=type==='sig'?1.8:1.15,ax=baseAxes(0,1,0,ymax);ax.xmin=0;ax.xmax=1;let out=ax.out,dx=1/n,sum=0;
